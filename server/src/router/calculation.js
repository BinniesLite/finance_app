...
router.get('/total-income-by-walletid/:id', async (req, res) => {
...
router.get('/total-expense-by-walletid/:id', async (req, res) => {
    const id = req.params.id;
    try {
        const totalExpense = await prisma.transaction.aggregate({
            _sum: {
...
router.get('/balance-by-walletid/:id', async (req, res) => {
    const id = req.params.id;
    try {
        const totalIncome = await prisma.transaction.aggregate({
            _sum: {
...
router.get('/total-income-by-date', async (req, res) => {
...
// note: this get request requires query parameters.
// date format: YYYY-MM-DD
router.get('/total-expense-by-date', async (req, res) => {
    const start = req.query.start;
    const end = req.query.end;
    const startDate = start + 'T00:00:00.000Z'; // add time to the date to make it ISO format as required by prisma
    const endDate = end + 'T23:59:59.999Z'; // add time to the date to make it ISO format as required by prisma
...
// note: this get request requires query parameters.
// date format: YYYY-MM-DD
router.get('/balance-by-date', async (req, res) => {
    const start = req.query.start;
    const end = req.query.end;
    const startDate = start + 'T00:00:00.000Z'; // add time to the date to make it ISO format as required by prisma
    const endDate = end + 'T23:59:59.999Z'; // add time to the date to make it ISO format as required by prisma
